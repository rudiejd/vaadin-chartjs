...
public abstract class BaseScale<B extends BaseScale<?>> implements JsonBuilder, Serializable {
  ...
  /**
   * If true, show the scale including gridlines, ticks, and labels. Overrides gridLines.display,
  ...
  /**
   * The ID is used to link datasets and scale axes together. The properties `datasets.xAxisID` or
   * `datasets.yAxisID` have to match the scale properties `scales.xAxes.id` or `scales.yAxes.id`.
  ...
  /**
   * Set this to ensure that the automatically sized bars are not sized thicker than this. Only
   * works if barThickness is not set (automatic sizing is enabled).
   */
  ...
  /**
   * Percent (0-1) of the available width each bar should be within the category percentage. 1.0
