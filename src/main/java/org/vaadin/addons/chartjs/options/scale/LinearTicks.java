...
public class LinearTicks<T> extends Ticks<T> implements JsonBuilder {
  ...
  /**
   * Maximum number of ticks and gridlines to show. If not defined, it will limit to 11 ticks but
   * will show all gridlines.
   */
  ...
  /**
   * User defined fixed step size for the scale. If set, the scale ticks will be enumerated by
   * multiple of stepSize, having one tick per increment. If not set, the ticks are labeled
   * automatically using the nice numbers algorithm.
   */
  ...
  /**
   * if defined, it can be used along with the min and the max to give a custom number of steps. See
   * the example below.
   */
  ...
  /**
   * User defined maximum number for the scale, overrides maximum value except for if it is lower
  ...
  /**
   * User defined minimum number for the scale, overrides minimum value except for if it is higher
