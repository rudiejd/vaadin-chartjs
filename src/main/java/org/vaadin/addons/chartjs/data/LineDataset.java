...
public class LineDataset extends DoubleDataset<LineDataset> {
   ...
   *
   * <p>The 'default' algorithm uses a custom weighted cubic interpolation, which produces pleasant
   * curves for all types of datasets.
   *
   * <p>The 'monotone' algorithm is more suited to y = f(x) datasets. It preserves monotonicity (or
   * piecewise monotonicity) of the dataset being interpolated, and ensures local extremums (if any)
   * stay at input data points.
   */
  ...
  /**
   * The style of point. Options are 'circle', 'triangle', 'rect', 'rectRot', 'cross', 'crossRot',
   * 'star', 'line', and 'dash'.
   */
  ...
  public JsonObject buildJson() {
    ...
    if (cubicInterpolationMode != null) {
      JUtils.putNotNull(
          map, "cubicInterpolationMode", cubicInterpolationMode.toString().toLowerCase());
    }
    ...
    JUtils.putNotNullStringListOrSingle(
        map, "pointHoverBackgroundColor", pointHoverBackgroundColor);
